package com.example.pass.activities.analyseOffice.view;
...
import com.example.pass.activities.analyseOffice.adapter.LineAdapter;
import com.example.pass.activities.analyseOffice.model.impls.IOfficeModel;
import com.example.pass.activities.analyseOffice.presenter.AnalyseOfficePresenter;
...
public class AnalyseOfficeActivity extends BaseActivity<ITitleSelectView, AnalyseOfficePresenter<ITitleSelectView>> implements ITitleSelectView, Updatable {
    ...
    private void initData() {
        ...
        if (path != null) {
            ...
            mPresenter.readFileAndGetLineList(path, "Pass/read_temp/",singleName );
        }else{
            //不允许继续下去
        }
    }
    ...
    public void onFinishAndCompileListToXml() {
        if (path == null) return;
        mPresenter.compileLinesToXml("Pass/finished_xmls/", singleName, new IOfficeModel.OnProgressListener() {
            @Override
            public void onStart() {
                runOnUiThread(new Runnable() {
                    @Override
